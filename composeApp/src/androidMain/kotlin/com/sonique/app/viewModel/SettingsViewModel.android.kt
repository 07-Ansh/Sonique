...
import com.sonique.common.DOWNLOAD_EXOPLAYER_FOLDER
import com.sonique.common.EXOPLAYER_DB_NAME
...
import com.sonique.media3.di.stopService
import com.sonique.app.extension.bytesToMB
import com.sonique.app.extension.getSizeOfFile
import com.sonique.app.extension.zipInputStream
import com.sonique.app.extension.zipOutputStream
...
import sonique.composeapp.generated.resources.restore_success
import java.io.File
...
actual suspend fun calculateDataFraction(cacheRepository: CacheRepository): SettingsStorageSectionFraction? {
    ...
    return withContext(Dispatchers.Default) {
        ...
        if (mStorageStatsManager != null) {
            ...
            val soniqueSize = getSizeOfFile(application.filesDir).bytesToMB()
            val thumbSize = (application.imageLoader.diskCache?.size ?: 0L).bytesToMB()
            val otherApp = soniqueSize.let { usedSpace.minus(it) - thumbSize }
            val databaseSize =
                soniqueSize - playerCache.bytesToMB() - downloadCache.bytesToMB() - canvasCache.bytesToMB()
            if (totalByte ==
                freeSpace + otherApp + soniqueSize + thumbSize
            ) {
