...
import com.sonique.domain.manager.DataStoreManager.Values.LOCAL_PLAYLIST_FILTER_OLDER_FIRST
import com.sonique.domain.manager.DataStoreManager.Values.LRCLIB
import com.sonique.domain.manager.DataStoreManager.Values.PROXY_TYPE_HTTP
import com.sonique.domain.manager.DataStoreManager.Values.PROXY_TYPE_SOCKS
import com.sonique.domain.manager.DataStoreManager.Values.REPEAT_ALL
import com.sonique.domain.manager.DataStoreManager.Values.REPEAT_MODE_OFF
import com.sonique.domain.manager.DataStoreManager.Values.REPEAT_ONE
...
import com.sonique.common.QUALITY as COMMON_QUALITY
...
internal class DataStoreManagerImpl(
    ...
    override suspend fun setCrossfadeDjMode(enabled: Boolean) {
        withContext(Dispatchers.IO) {
            if (enabled) {
                settingsDataStore.edit { settings ->
                    settings[CROSSFADE_DJ_MODE] = TRUE
                }
            } else {
                settingsDataStore.edit { settings ->
                    settings[CROSSFADE_DJ_MODE] = FALSE
                }
            }
        }
    }
