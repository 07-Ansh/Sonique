...
actual class Extractor {
    ...
    actual fun mergeAudioVideoDownload(filePath: String): DownloadProgress {
        val command =
            listOf(
                ...
                "1:a:0",
                "-shortest",
                "$filePath-Sonique.mp4",
            ).joinToString(" ")

        if (FileSystem.SYSTEM.exists("$filePath-Sonique.mp4".toPath())) {
            FileSystem.SYSTEM.delete("$filePath-Sonique.mp4".toPath())
        }
    ...
    actual fun saveAudioWithThumbnail(
        ...
        try {
            ...
            if (FileSystem.SYSTEM.exists("$filePath-com.sonique.com.sonique.app.mp3".toPath())) {
                FileSystem.SYSTEM.delete("$filePath-com.sonique.com.sonique.app.mp3".toPath())
            }
        } catch (e: IOException) {
            e.printStackTrace()
        }
        ...
        val commandInject =
            listOf(
                ...
                "$filePath-com.sonique.com.sonique.app.mp3",
            ).joinToString(" ")
        val sessionInject =
            FFmpegKit.execute(
                commandInject,
