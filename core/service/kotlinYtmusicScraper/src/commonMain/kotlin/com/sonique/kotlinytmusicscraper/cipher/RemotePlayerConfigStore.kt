...
package com.sonique.kotlinytmusicscraper.cipher
...
/**
 * Runtime store for player configs.
 *
 * The store deliberately separates the normal six-hour refresh from the two
 * failure-triggered refreshes: an unknown player hash and a CDN rejection.
 * Both failure paths are single-flight and rate-limited so a bad player rotation
        ...
        runCatching { Url(value) }
            ...
            ?.takeIf { url ->
                url.protocol.name == "https" &&
                    when (url.host) {
                        // Published table registries and fallbacks
